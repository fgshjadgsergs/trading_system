...
LiqPriceFn = Callable[[float, float, Side], float]
...
class LiqMap:
    ...
    # -- update ---------------------------------------------------------------
    def allocate(
        self, d_oi_usd: float, price: float, context: Context | None = None
    ) -> None:
        ...
        for side, share in ((Side.BUY, self.long_share), (Side.SELL, 1.0 - self.long_share)):
            if share == 0.0:
                continue
    ...
    def step(
        ...
        self.allocate(d_oi_usd, bar_close, context)
        self.decay(dt_s)
